...
def svd():
    ...
    numM = 17770
    numE = 94362233
    #cdef np.ndarray[double, ndim=1] temp = np.zeros((numF), 'float')
    #cdef np.ndarray[double, ndim=1] movie_a = np.zeros(numM, 'float')
    ...
    #userFeature = np.zeros((numU, numF), 'float') + 0.1
    #movieFeature = np.zeros((numM, numF), 'float') + 0.1
    #userFeature = iom.read('uf', 'float')
    #movieFeature = iom.read('mf', 'float')
    ...
    #userTemp = iom.read('../binary/user.dta', 'uint')
    userTemp = np.fromfile('../binary/user.dta', 'uint')
    movieTemp = np.fromfile('../binary/movie.dta', 'uint')
    #movieTemp = iom.read('../binary/movie.dta', 'uint')
    #ratingTemp = iom.read('../binary/date.dta', 'uint')
    ...
    #iom.printToTxt(movieTemp, 'as.txt')
